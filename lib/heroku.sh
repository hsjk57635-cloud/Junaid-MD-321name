...
# Usage: bash <(curl -s https://raw.githubusercontent.com/JUNAID-MD_³⁰²Info/MEGA-MDX/main/lib/heroku.sh)
...
# Session ID
read -p "$(echo -e ${CYAN}Session ID (JUNAID-MD_³⁰²Info/MEGA-MD_xxxxx): ${NC})" SESSION_ID
...
# Owner number
read -p "$(echo -e ${CYAN}Owner WhatsApp number (e.g. 923442174513): ${NC})" OWNER_NUMBER
OWNER_NUMBER=${OWNER_NUMBER:-923442174513}
...
if [ ! -f "heroku.yml" ]; then
    ...
    git clone https://github.com/JUNAID-MD_³⁰²Info/MEGA-MDX mega-md-deploy
    cd mega-md-deploy
fi
