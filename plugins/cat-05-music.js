'use strict';
/*****************************************************************************
 *  cat-05-music.js — JUNAID-MD_³⁰² Music Bundle v5 ULTRA
...
const _play = {
    ...
    category: 'music', description: 'Search YouTube and choose Music, Video or Voice',
    ...
    async handler(sock, message, args, context = {}) {
        ...
        try {
            ...
            const video = await ytSearch(query);
            const token = encodePlayQuery(query);
            ...
            const buttons = [
                {
                    name: 'quick_reply',
                    buttonParamsJson: JSON.stringify({ display_text: '🎵 Music', id: `JPLAY:AUDIO:${token}` })
                },
                {
                    name: 'quick_reply',
                    buttonParamsJson: JSON.stringify({ display_text: '🎬 Video', id: `JPLAY:VIDEO:${token}` })
                },
                {
                    name: 'quick_reply',
                    buttonParamsJson: JSON.stringify({ display_text: '🎙️ Voice', id: `JPLAY:VOICE:${token}` })
                }
            ];

            await sendInteractiveMessage(sock, chatId, {
                text: `🎵 *${video.title}*\n\nChoose how you want this song:`,
                footer: 'JUNAID-MD_³⁰² • YouTube Play',
...
async function handlePlayButtonResponse(sock, message, context = {}) {
    try {
        ...
        let params = {};
        const rawParams = native.paramsJson || native.messageParamsJson || native.params || '{}';
        try { params = typeof rawParams === 'string' ? JSON.parse(rawParams) : (rawParams || {}); } catch (_) {}
        const id = String(params.id || params.selectedId || params.buttonId || params.selectedButtonId || native.id || '');
        if (!id.startsWith('JPLAY:')) return false;
